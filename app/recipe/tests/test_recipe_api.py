...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe_with_ingredient(self):
        ...
        ingredient1 = sample_ingredient(user = self.user, name = 'Zuccini')
...
class RecipeImageUploadTests(TestCase):
    def setUp(self):
        ...
        self.client.force_authenticate(self.user)
        self.recipe = sample_tag(user = self.user)
    
    def tearDown(self):
        """removing all test files that we created"""
        self.recipe.image.delete()
    
    def test_uploading_image_to_recipe(self):
        """test uploading to recipe """
        url = image_upload_url(self.recipe.id)
        with tempfile.NamedTemporaryFile(suffix='.jpg') as ntf:
            img = Image.new('RGB', (10, 10))
            img.save(ntf, format='JPEG')
            # because we saved the file, the seeking will be done to the end
            # if we tried to access it would be blank because it's already at the end of the file
            # so we need to pointer back to the beginning of the file
            ntf.seek(0)
